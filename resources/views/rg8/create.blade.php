...
@section('content')
        ...
        <p class="text-gray-500 mb-6">Gestion des paiements d'irrigation</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 bg-white p-6 rounded-lg shadow-md">
                    ...
                    <div class="relative mb-4">
                        <input type="text" id="search-client-input" placeholder="Rechercher par code client..." class="w-full p-2 border rounded-md">
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="client-code" class="w-full p-2 bg-gray-100 border-gray-200 rounded-md" readonly>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Nom et Prénom</label>
                            <input type="text" id="client-name" class="w-full p-2 bg-gray-100 border-gray-200 rounded-md" readonly>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-600">Téléphone</label>
                            <input type="text" id="client-phone" class="w-full p-2 bg-gray-100 border-gray-200 rounded-md" readonly>
                        </div>

                        <div>
                            <label for="date-operation" class="block text-sm font-medium text-gray-600">Date</label>
                            <input type="date" id="date-operation" name="date_operation" value="{{ date('Y-m-d') }}" class="w-full p-2 border border-gray-300 rounded-md">
                        </div>

                        <div>
                            <label for="secteur" class="block text-sm font-medium text-gray-600">Secteur</label>
                            <input type="text" id="secteur" name="secteur" class="w-full p-2 bg-gray-100 border-gray-200 rounded-md" readonly>
                        </div>

                        <div>
                            <label for="numero-recu" class="block text-sm font-medium text-gray-600">N° Reçu</label>
                            <input type="text" id="numero-recu" name="numero_recu" class="w-full p-2 border border-gray-300 rounded-md" placeholder="Entrez le n° du reçu...">
                        </div>

                        <div class="sm:col-span-2">
                            <label for="numero-rg8" class="block text-sm font-medium text-gray-600">Numéro RG8</label>
                            <div class="flex">
                                <input type="text" id="numero-rg8" name="numero_rg8" class="w-full p-2 border border-gray-300 rounded-l-md">
                                <button type="button" class="bg-gray-700 text-white px-4 rounded-r-md hover:bg-gray-800">+</button>
                            </div>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="flex flex-wrap gap-x-6 gap-y-2">
                                <div class="flex items-center">
                                    <input id="reg-espece" value="espece" name="methode_reglement" type="radio" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300" checked>
                                    <label for="reg-espece" class="ml-2 block text-sm text-gray-900">Espèce</label>
                                </div>
                                <div class="flex items-center">
                                    <input id="reg-cheque" value="cheque" name="methode_reglement" type="radio" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                                    <label for="reg-cheque" class="ml-2 block text-sm text-gray-900">Chèque</label>
                                </div>
                                <div class="flex items-center">
                                    <input id="reg-virement" value="virement" name="methode_reglement" type="radio" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                                    <label for="reg-virement" class="ml-2 block text-sm text-gray-900">Virement</label>
                                </div>
                                <div class="flex items-center">
                                    <input id="reg-versement" value="versement" name="methode_reglement" type="radio" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function resetForm() {
                ...
                facturesListContainer.innerHTML = '<p class="text-gray-500">Veuillez rechercher un client pour voir ses factures.</p>';
                selectedFactures = [];
                updateRecap();
            }
            ...
            recapTableBody.addEventListener('click', function(event) {
                if (event.target && event.target.classList.contains('remove-facture-btn')) {
                    ...
                    const addButton = facturesListContainer.querySelector(`.add-facture-btn[data-id='${removedFactureId}']`);
                    if (addButton) {
            ...
            searchInput.addEventListener('change', function() {
                ...
                resetForm(); // Reset on new search
                searchInput.value = query;

                if (query.length > 0) {
                    const url = `{{ route('clients.search') }}?query=${query}`;
                    fetch(url)
                        .then(response => { if (!response.ok) throw new Error('Client non trouvé'); return response.json(); })
                        .then(data => {
                            document.getElementById('client-id').value = data.client.id;
                            document.getElementById('client-code').value = data.client.code_client;
                            document.getElementById('client-name').value = data.client.nom_client + ' ' + data.client.prenom_client;
                            ...
                            if (data.factures.length > 0) {
                                ...
                                facturesListContainer.innerHTML = '<p class="text-gray-500">Ce client n\'a aucune facture non payée.</p>';
                            }
                        })
                        .catch(error => {
                            alert(error.message);
